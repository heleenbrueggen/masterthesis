...
imp_stan4bart <- list()
for (i in seq_len(nrow(combinations))) {
  imp_stan4bart[[i]] <- map(read_rds(paste("/Volumes/Heleen\ 480GB/Master\ thesis/results/imputed/stan4bart/results_stan4bart_", names[i], ".rds", sep = "")), ~.x$imp)
}
############################
...
for (i in seq_len(nrow(combinations))) {
    ...
    # Save results
    write_rds(analyses_stan4bart, paste("/Volumes/Heleen\ 480GB/Master\ thesis/results/imputed/stan4bart/analyses_stan4bart_", names[i], ".rds", sep = ""))
}
############################
