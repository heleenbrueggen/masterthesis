...
imp_bart <- list()
for (i in seq_len(nrow(combinations))) {
  imp_bart[[i]] <- map(read_rds(paste("results/imputed/bart/results_bart_", names[i], ".rds", sep = ""))[1:100], ~.x$imp)
}
############################
...
for (i in seq_len(nrow(combinations))) {
    ...
    # Save results
    write_rds(analyses_bart, paste("results/imputed/bart/analyses_bart_", names[i], ".rds", sep = ""))
}
############################
