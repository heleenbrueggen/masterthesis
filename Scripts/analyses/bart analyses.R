...
for (i in seq_len(nrow(combinations))) {
  imp_bart[[i]] <- map(read_rds(paste(path, "results/imputed/bart/results_bart_", names[i], ".rds", sep = "")), ~.x$imp)
}
############################
...
for (i in seq_len(nrow(combinations))) {
    ...
    # Perform analyses
    analyses_bart <- pblapply(imp_bart[[i]], lmer.model, cl = cl)
    # Save results
    write_rds(analyses_bart, paste(path, "results/imputed/bart/analyses_bart_", names[i], ".rds", sep = ""))
}
############################
