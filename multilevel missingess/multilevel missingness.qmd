...
bias_ld <- map(bias.datasets_ld, \(x) x$bias) %>%
    list_rbind() %>%
    as_tibble() %>%
    format.bias(combinations, method = "ld", name = "Bias") %>%
    cbind(map(bias.datasets_ld, \(x) x$bias.mcse) %>%
        list_rbind() %>%
        as_tibble() %>%
        format.bias(combinations, method = "ld", name = "MCSE") %>%
        dplyr::select(MCSE))
```
...
#| echo: false
#| fig.cap: "Bias of the estimates for all datasets"
