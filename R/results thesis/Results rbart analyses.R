...
results_rbart <- list()
for (i in seq_len(nrow(combinations))) {
  results_rbart[[i]] <- read_rds(paste("/Volumes/Heleen\ 480GB/Master\ thesis/results/imputed/rbart/analyses_rbart_", names[i], ".rds", sep = ""))[1:100]
}
#####################################
...
##############
# Bias
bias.datasets_rbart <- list()
for (i in seq_len(nrow(combinations))) {
    # Logging iteration
    cat("Processing iteration:", i, "\n")
    # Bias
    bias.datasets_rbart[[i]] <- bias(results_rbart[[i]])
}
...
for (i in seq_len(nrow(combinations))) {
    ...
    cat("Processing iteration:", i, "\n")
    # Coverage
    coverage.datasets_rbart[[i]] <- coverage(results_rbart[[i]])
}
...
for (i in seq_len(nrow(combinations))) {
    ...
    cat("Processing iteration:", i, "\n")
    # CIW
    ciw.datasets_rbart[[i]] <- ciw(results_rbart[[i]])
}
ciw_rbart <- map(ciw.datasets_rbart, ~ .x$ciw) %>% list_rbind()
# Saving
write_rds(ciw.datasets_rbart, file = "/Volumes/Heleen\ 480GB/Master\ thesis/results/evaluations/ciw_datasets_rbart.rds")
write_rds(ciw_rbart, file = "/Volumes/Heleen\ 480GB/Master\ thesis/results/evaluations/ciw_rbart.rds")