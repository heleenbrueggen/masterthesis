...
\label{eq:population}
\begin{align}
        y_{ij} &= \beta_{0j} + \sum_{k=1}^{7}\beta_{kj}X_{kij} + \epsilon_{ij}, \hspace{3cm} X_{kij} \sim \mathcal{MVN}(0, \boldsymbol{\Sigma}_{x}), \hspace{.2cm} \epsilon_{ij} \sim \mathcal{N}(0, 25), \\
        &\hspace{.5cm}\beta_{0j} = \gamma_{00} + \sum_{q=1}^{2}\gamma_{0q}Z_{qj} + \upsilon_{0j}, \\
        &\hspace{.5cm}\beta_{kj} = \gamma_{k0} + \sum_{q=1}^{2}\gamma_{kq}Z_{qj} + \upsilon_{kj}, \hspace{2cm} Z_{qj} \sim \mathcal{MVN}(0, \boldsymbol{\Sigma}_{z}), \hspace{.2cm} \boldsymbol{\upsilon}_{j} \sim \mathcal{MVN}(0, \mathbf{T}), \\
...
\end{enumerate} The second and third methods will be implemented with the R-package \texttt{MICE} \citep{buuren2011}. 

The fourth, single-level BART, fifth, random intercept BART and sixth method, multilevel BART methods will be implemented by writing functions in R \citep{rcoreteam2023} for the package \texttt{MICE} \citep{buuren2011}. The functions \texttt{bart} and \texttt{rbart\_vi} from the \texttt{dbarts} package are used for the single-level and random intercept BART imputation methods. The function \texttt{stan4bart} from the package \texttt{stan4bart} is used for the multilevel BART imputation method accounting for random effects and cross-level interactions. The functions are written such that they can be used as imputation method in the \texttt{mice} package.
...
\end{align} where $\hat{\theta}_t$ is the estimated parameter in simulation \textit{t}, $\theta$ is the true value, and $n_{\text{sim}}$ is the number of simulated datasets and smaller is better.
